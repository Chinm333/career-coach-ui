...
export default function RegisterCandidate() {
    ...
    const submit = async (e) => {
        ...
        setLoading(true);
        try {
            const r = await api.post("/auth/register/candidate", {
                email,
                password,
                name,
                linkedinUrl,
    ...
    return (
        <div className="flex justify-center items-center min-h-[80vh]">
            <div className="w-full max-w-md bg-white/90 rounded-2xl shadow-2xl p-8 border border-gray-100 animate-fade-in">
                <h2 className="text-3xl font-extrabold text-primary-dark mb-1 text-center drop-shadow-sm">Register as Candidate</h2>
                <div className="text-gray-500 font-medium mb-6 text-center">Create your account</div>
                ...
                <form onSubmit={submit} className="space-y-5">
                    <div className="relative">
                        <input value={name} onChange={e => setName(e.target.value)} placeholder=" " required className="peer w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:border-primary-dark focus:ring-2 focus:ring-primary/30 transition outline-none text-gray-800" />
                        <span className="absolute left-3 top-2 text-gray-500 text-xs pointer-events-none transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400 peer-focus:top-2 peer-focus:text-xs">Full Name</span>
                    </div>
                    <div className="relative">
                        <input value={linkedinUrl} onChange={e => setLinkedinUrl(e.target.value)} placeholder=" " required className="peer w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:border-primary-dark focus:ring-2 focus:ring-primary/30 transition outline-none text-gray-800" />
                        <span className="absolute left-3 top-2 text-gray-500 text-xs pointer-events-none transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400 peer-focus:top-2 peer-focus:text-xs">LinkedIn URL</span>
                    </div>
                    <div className="relative">
